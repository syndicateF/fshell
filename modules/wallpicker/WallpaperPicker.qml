...
Item {
    ...
    readonly property real unselectedScale: 0.5
    readonly property real delegateWidth: cardWidth + cardSpacing

    property bool initialFocusSet: false

    // ─── Scroll & animation throttle (sama seperti referensi) ────
    ...
    // Fallback: Loader di Wrapper aktif setelah signal wallpickerChanged fire,
    // jadi Component.onCompleted adalah entry point yang lebih reliable.
    ...
    ListView {
        ...
        header: Item { width: Math.max(0, (carousel.width / 2) - ((root.cardWidth * root.selectedScale) / 2)) }
        footer: Item { width: Math.max(0, (carousel.width / 2) - ((root.cardWidth * root.selectedScale) / 2)) }
        ...
        // Tandai animasi sedang berjalan setiap kali index berubah
        onCurrentIndexChanged: {
            root.isItemAnimating = true;
            itemAnimationTimer.restart();
        }

        // Mouse wheel navigation — pakai scrollThrottle agar tidak spam
        MouseArea {
            anchors.fill: parent
            ...
            onWheel: wheel => {
                ...
                if (Math.abs(root.scrollAccum) >= root.scrollThreshold) {
                    if (root.scrollAccum > 0 && carousel.currentIndex > 0)
                        carousel.currentIndex--;
                    else if (root.scrollAccum < 0 && carousel.currentIndex < carousel.count - 1)
                        carousel.currentIndex++;
                    root.scrollAccum = 0;
        ...
        delegate: Item {
            ...
            // readonly property real targetWidth: delegateRoot.isCurrent
            //     ? (root.cardWidth * root.selectedScale + root.cardSpacing)
            //     : 130

            readonly property real targetWidth: delegateRoot.isCurrent
                ? (root.cardWidth * root.selectedScale)
                : (root.cardWidth * root.unselectedScale)

            width: targetWidth + root.cardSpacing
            ...
            // Visual card (scale applied here, NOT on delegate root)
            Item {
                id: cardVisual

                anchors.centerIn: parent
                width: delegateRoot.width - root.cardSpacing
    ...
    // ─── Keyboard navigation — Shortcut agar tidak fire on key repeat ─
    Shortcut {
        sequence: "Left"
        enabled: root.initialFocusSet
        onActivated: {
    ...
    Shortcut {
        sequence: "Escape"
        onActivated: root.visibilities.wallpicker = false
    }
    Shortcut {
        sequence: "Return"
        enabled: root.initialFocusSet
